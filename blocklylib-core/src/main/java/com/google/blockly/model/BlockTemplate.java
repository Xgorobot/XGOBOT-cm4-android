...
public class BlockTemplate {
    ...
    /*可从外部调用的参数更改方法
     仅适合生成动作编程中的input_position
     需在生成block之前完成
     which:1 2 3 4
     top middle bottom 1-255*/
    public BlockTemplate setmFieldValues(String which, String top, String middle, String bottom){
        if (mFieldValues == null) {
            mFieldValues = new ArrayList<>();
        }
        mFieldValues.add(new FieldValue("which",which));
        mFieldValues.add(new FieldValue("top",top));
        mFieldValues.add(new FieldValue("middle",middle));
        mFieldValues.add(new FieldValue("bottom",bottom));
        return this;
    }
